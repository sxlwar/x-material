...
import {
    ComponentFactoryResolver, ComponentRef, EmbeddedViewRef, Injectable, Injector, Optional,
    Provider, SkipSelf, TemplateRef, ViewContainerRef
} from '@angular/core';

import { LoadingStyle, XMatLoadingComponent } from './loading.component';
import { XMatLoadingContext } from './loading.directive';
import { IXMatLoadingConfig } from './loading.service';

export interface IInternalLoadingOptions extends IXMatLoadingConfig {
  height?: number;
  style?: LoadingStyle;
}
 ...
 */
@Injectable()
export class XMatLoadingFactory {
  constructor(
   ...
   */
  public createFullScreenComponent(options: IXMatLoadingConfig): ILoadingRef {
    ...
    loadingRef.observable.pipe(distinctUntilChanged()).subscribe((registered: number) => {
      if (registered > 0 && !loading) {
        ...
        loadingRef.componentRef = overlayRef.attach(new ComponentPortal(XMatLoadingComponent));
  ...
  private _createComponent(options: IInternalLoadingOptions): ILoadingRef {
    ...
    compRef.componentRef = this._componentFactoryResolver
      .resolveComponentFactory(XMatLoadingComponent)
   ...
   */
  private _mapOptions(options: IInternalLoadingOptions, instance: XMatLoadingComponent): void {
...
export function LOADING_FACTORY_PROVIDER_FACTORY(
  parent: XMatLoadingFactory,
  componentFactoryResolver: ComponentFactoryResolver,
  overlay: Overlay,
  injector: Injector
): XMatLoadingFactory {
  return parent || new XMatLoadingFactory(componentFactoryResolver, overlay, injector);
}

export const LOADING_FACTORY_PROVIDER: Provider = {
  // If there is already a service available, use that. Otherwise, provide a new one.
  provide: XMatLoadingFactory,
  deps: [[new Optional(), new SkipSelf(), XMatLoadingFactory], ComponentFactoryResolver, Overlay, Injector],
